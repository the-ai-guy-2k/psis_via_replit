...
- **Game Status Panel**: replaces the old static helper-text description under "Log At-Bat Outcome" on the Tracker header. Shows current Inning, Outs, Current Δ (all sourced from the same `GET /innings/current` data as before, no API change), plus a "Completed" inning-delta history (e.g. "1: +1 2: +3 3: -2"). The completed history is computed purely client-side in `track.tsx` (`computeCompletedInningDeltas`) from the full `GET /entries` list — an inning counts as complete once its at-bats' `outsAdded` sum to 3, mirroring the server's own completion rule without a new endpoint. This absorbed the old separate right-aligned "inning status bar" box (Inning/Outs/Inning Δ), which no longer exists as a standalone element.
